...
const validateFiledata = (filepath, filename) => {
    if (!filepath) throw new Error(`The file path to  ${filename} was not provided.`);
    if (!filename) throw new Error(`The file name ${filename} was not provided.`);
};


export const readJsonFile = async (filepath, filename) => {
    validateFileData(filepath, filename);
